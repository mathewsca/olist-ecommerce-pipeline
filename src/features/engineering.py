            ...
            if col in df.columns:
                for window in window_sizes:
                    for op in operations:
                        ...
                        if op == 'mean':
                            df_rolling[feature_name] = df[col].rolling(window=window).mean()
                        elif op == 'std':
                            df_rolling[feature_name] = df[col].rolling(window=window).std()
                        elif op == 'min':
                            df_rolling[feature_name] = df[col].rolling(window=window).min()
                        elif op == 'max':
                            df_rolling[feature_name] = df[col].rolling(window=window).max()
    ...
    def create_binning_features(self, df: pd.DataFrame,
        ...
        for col in columns:
            if col in df.columns and df[col].dtype in ['int64', 'float64']:
                ...
                if method == 'quantile':
                    df_binned[bin_col_name] = pd.qcut(df[col], q=bins, labels=False, duplicates='drop')
                elif method == 'uniform':
                ...
                for group_col in groupby_columns:
                    if group_col in df.columns:
                        for op in operations:
                            ...
                            if op == 'mean':
                                df_stats[feature_name] = df.groupby(group_col)[col].transform('mean')
                            elif op == 'std':
                                df_stats[feature_name] = df.groupby(group_col)[col].transform('std')
                            elif op == 'min':
                                df_stats[feature_name] = df.groupby(group_col)[col].transform('min')
                            elif op == 'max':
                                df_stats[feature_name] = df.groupby(group_col)[col].transform('max')
                            elif op == 'count':
                                df_stats[feature_name] = df.groupby(group_col)[col].transform('count')

...
class DataVisualizer:
    ...
    def plot_distribution(self, df: pd.DataFrame, columns: List[str] = None,
                         plot_type: str = 'histogram', bins: int = 30,
        ...
        for i, col in enumerate(columns):
            if plot_type == 'histogram':
                axes[i].hist(df[col].dropna(), bins=bins, alpha=0.7, color=self.colors[i % len(self.colors)])
            elif plot_type == 'kde':
                df[col].dropna().plot(kind='kde', ax=axes[i], color=self.colors[i % len(self.colors)])
            elif plot_type == 'box':
    ...
    def plot_correlation_matrix(self, df: pd.DataFrame, 
        ...
        # Create heatmap
        plt.figure(figsize=(max(8, len(corr_matrix.columns) * 0.8), 
    ...
    def plot_model_performance(self, metrics: Dict[str, Any],
    ...
    def plot_time_series(self, df: pd.DataFrame, time_column: str,
        ...
        for i, col in enumerate(value_columns):
            if col in df.columns:
                plt.plot(df[time_column], df[col], label=col, color=self.colors[i % len(self.colors)])
    ...
    def plot_missing_data(self, df: pd.DataFrame,
        ...
        # Missing count plot
        sns.barplot(data=missing_df, x='Missing Count', y='Column', ax=axes[0], palette='viridis')
        axes[0].set_title('Missing Data Count by Column')
        
        # Missing percentage plot
        sns.barplot(data=missing_df, x='Missing Percent', y='Column', ax=axes[1], palette='viridis')

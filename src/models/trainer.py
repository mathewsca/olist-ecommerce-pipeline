...
class ModelTrainer:
    ...
    def train_model(self, X_train: pd.DataFrame, y_train: pd.Series,
                   algorithm: str = 'random_forest', **params) -> Any:
        """
    ...
    def hyperparameter_tuning(self, X_train: pd.DataFrame, y_train: pd.Series,
                             algorithm: str, param_grid: Dict[str, List],
                             cv: int = 5) -> Any:
        """
        ...
        grid_search = GridSearchCV(
            base_model, param_grid, cv=cv, scoring='accuracy' if self.model_type == 'classification' else 'r2'
        )
    ...
    def predict_proba(self, model: Any, X: pd.DataFrame) -> np.ndarray:
        ...
        """
        if self.model_type != 'classification':
            raise ValueError("Probability prediction only available for classification models")

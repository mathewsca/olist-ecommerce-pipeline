...
class FabricaIAPipeline:
    ...
    def load_and_explore_data(self, data_path: str) -> pd.DataFrame:
        ...
        if numerical_cols:
            self.visualizer.plot_distribution(df, numerical_cols[:6], save_path="logs/distributions.png")
        
        # Plot correlation matrix
        if len(numerical_cols) > 1:
            self.visualizer.plot_correlation_matrix(df, save_path="logs/correlation_matrix.png")
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(numerical_cols) >= 2:
            df_poly = self.feature_engineer.create_polynomial_features(
                df, numerical_cols[:2], degree=self.config['FEATURES']['polynomial_degree']
            )
        else:
            df_poly = df
        ...
        if len(numerical_cols) >= 2:
            ...
            df_interact = self.feature_engineer.create_interaction_features(df_poly, interaction_pairs)
        else:
            df_interact = df_poly
    ...
    def train_models(self, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        for algorithm in algorithms:
            ...
            # Save model
            model_path = f"{self.config['MODEL_PATHS']['trained']}/{algorithm}_model.pkl"

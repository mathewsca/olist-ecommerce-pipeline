...
def preprocess_data(**context):
    ...
    df = data_processor.load_data(data_path)
    df_clean = data_processor.clean_data(df, drop_duplicates=True, handle_missing='fill')
...
def engineer_features(**context):
    ...
    if len(numerical_cols) >= 2:
        ...
        df_interact = feature_engineer.create_interaction_features(df_poly, [(numerical_cols[0], numerical_cols[1])])
    else:
        df_interact = df
...
def evaluate_models(**context):
    ...
    # Log best model
    best_model = max(results.keys(), key=lambda x: results[x].get('accuracy', results[x].get('r2_score', 0)))
...
# Task dependencies
load_data_task >> preprocess_task >> engineer_features_task >> train_models_task >> evaluate_models_task >> cleanup_task

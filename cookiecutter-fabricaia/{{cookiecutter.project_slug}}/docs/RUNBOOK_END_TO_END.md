# Runbook de Execução End-to-End — {{cookiecutter.project_name}}
## Guia Operacional Prático para Desenvolvimento, Serving e MLOps

Este documento fornece um guia passo a passo prático para experimentar, executar e reproduzir o ciclo de vida completo entregue pelo projeto **{{cookiecutter.project_name}}**, desde o scaffolding inicial e engenharia de dados até o serving em produção com combate ao *training-serving skew*, governança técnica e orquestração contínua de MLOps.
...
Para demonstrar os recursos do projeto com dados estruturados realistas, utilizamos como estudo de caso a **predição de risco de atraso e paralisação de contratos públicos**:
...
|                                  FLUXO END-TO-END                                       |
|                                                                                         |
|  [Geração de Dados] ─────────> [Treino & MLflow] ─────────> [Serving REST FastAPI]      |
|           │                            │                               │                |
|    make data (CSV)               make pipeline                make api (Anti-Skew)      |
|                                        │                               │                |
|                                [Airflow MLOps] <───────────────────────┘                |
|                               (Drift, Store, Gate)                                      |
+-----------------------------------------------------------------------------------------+
...
MLFLOW:
  ...
  experiment_name: "{{cookiecutter.project_slug}}_experiments"
...
No projeto, adotamos a separação padrão da indústria entre **Serving Online** e **Pipelines Offline**:
...
1. **A API trata dados brutos *On the Fly* (em tempo de execução):**
   ...
   * Por isso, a camada `PredictionService` (`src/services/prediction_service.py`) recebe os atributos brutos, aplica a limpeza, codificação categórica, escalonamento, gera as variáveis polinomiais/interações e alinha estritamente as colunas com `model.feature_names_in_`.
...
docker run -p 8000:8000 -p 5001:5001 {{cookiecutter.project_slug}}
```
...
Para garantir conformidade regulatória, rastreabilidade e explicabilidade exigidas em auditorias e diretrizes éticas de IA, o projeto disponibiliza artefatos padronizados na pasta `docs/`:
...
| **Build Docker** | `make docker-build` | `docker build -t {{cookiecutter.project_slug}} .` | `docker build -t {{cookiecutter.project_slug}} .` |
| **Executar Contêiner Docker** | `make docker-run` | `docker run -p 8000:8000 -p 5001:5001 {{cookiecutter.project_slug}}` | `docker run -p 8000:8000 -p 5001:5001 {{cookiecutter.project_slug}}` |

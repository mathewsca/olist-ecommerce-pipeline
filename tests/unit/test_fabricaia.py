...
class TestDataProcessor:
    ...
    def test_clean_data_drop_duplicates(self):
        ...
        cleaned_data = self.processor.clean_data(data_with_duplicates, drop_duplicates=True)
    ...
    def test_clean_data_handle_missing(self):
        ...
        # Test drop strategy
        cleaned_drop = self.processor.clean_data(data_with_missing, handle_missing='drop')
        assert not cleaned_drop.isnull().any().any()
        
        # Test fill strategy
        cleaned_fill = self.processor.clean_data(data_with_missing, handle_missing='fill')
        assert not cleaned_fill.isnull().any().any()
    
    def test_encode_categorical(self):
        """Test categorical encoding."""
        encoded_data = self.processor.encode_categorical(self.sample_data, method='label')
        
        # Check that categorical column is encoded
        assert encoded_data['category'].dtype in ['int64', 'int32']
        assert len(encoded_data['category'].unique()) == len(self.sample_data['category'].unique())
